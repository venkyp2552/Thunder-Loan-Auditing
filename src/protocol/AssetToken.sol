...
contract AssetToken is ERC20 {
    ...
    // e underlaying== USDC (investments token)
    // e assestToken(shares) == returns in any token
    // e 2 USDC = 1 assetToken
    // q what does this s_exchangeRate do?
    // Q-Answered : the rate in in between underlaying and assettoken.
    ...
    constructor(
        ...
        // Oh, are the ERC20sstored in AssetToken.sol instead of Thunderloan.sol?
        // q Where are tokens stored?
        // q-Answered : In AssetToken contract only.
    ...
    //e ok , only the Tunderloan contract can mint assettokens
    ...
    function transferUnderlyingTo(address to, uint256 amount) external onlyThunderLoan {
        //weird ERC20?
        // q-answered what if USDC blacklists the tunderloan contract?
        // q-answered what if USDC blacklists the asset token contract?
        //@audit-Medium : the protocol will be frozen, thet would scuk
        i_underlying.safeTransfer(to, amount);
    }

    // e  responsible for updating exchange rate or fee AssetToken->Underlaying
    function updateExchangeRate(uint256 fee) external onlyThunderLoan {
        ...
        // q what if totalSupplly is 0? 
        // this breaks is that an issue?
        //@audit-gas, too many storage variables reads , store as a memeory
